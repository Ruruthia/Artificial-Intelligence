...
def start_benchmark():
    ...
    result = timeit.repeat(
        "validator.simple_benchmark()", setup="import validator", number=10, repeat=10
    )
...
if __name__ == "__main__":
    ...
    for case_num, case_def in problem_cases:
        ...
        try:
            ...
            case_meas = run_and_score_case(
                ...
                case_def,
                problem_validator,
                timeout_multiplier * benchmark_result,
            )
    ...
    if failed_cases:
        ...
        if args.testset:
            misc_opts = "%s --testset %s" % (misc_opts, shellquote(args.testset),)
        cases_opt = "--cases " + ",".join([str(fc) for fc in failed_cases])
        print(
            "python validator.py%s %s %s %s"
