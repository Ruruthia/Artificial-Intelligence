...
# gracz 1. na dole, gracz 0. u góry, pierwszy się rusza
class Jungle:
    def __init__(self):
        self.counter = 0
    ...
    def simulate_game(self, move, player):
        move_count = 1
        self.do_move(player, move)  # player to my
        curr_player = 1 - player
        while self.winner_withoutcount() == -1:
            move_count += 1
            move = self.random_move(curr_player)
            self.do_move(curr_player, move)
            curr_player = 1 - curr_player
        if self.winner_withoutcount() == player:
            # self.draw_board()
            # print()
            return 1, move_count
        else:
...
for i in tqdm(range(10)):
    while j.winner() == -1:
        if current_player == our_position:
            move = j.simulated_move(current_player, 50000)
        else:
            move = j.random_move(current_player)
