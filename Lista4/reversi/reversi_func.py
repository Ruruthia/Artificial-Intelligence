...
class Game:
    ...
    def heuristic(self, board, turn):
        ...
        params = [300 + 5 * turn, 200 + 4 * turn, 100 - turn]
        v = params[0] * c + params[1] * cp + params[2] * p_m

        return v

    def minimax_move(self, player, depth, board, alpha, beta, turn):

        best_move = None
        if depth == 0 or board.terminal():
            v = self.heuristic(board, turn)
            return v, best_move
        ...
        if player:
            ...
            for move in ms:
                ...
                new_value, _ = self.minimax_move(
                    1 - player, depth - 1, board, alpha, beta, turn + 1
                )
                board.reverse_move()
                if new_value > value:
                    value = new_value
        ...
        while not self.reversi.terminal():
            ...
            if self.player == self.our_position:
                val, move = self.minimax_move(
                    self.player, 1, self.reversi, -10000000, 10000000, turn
                )
            else:
